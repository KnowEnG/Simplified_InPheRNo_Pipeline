...
def verify_benchmark(option, BENCHMARK_name_list, BENCHMARK_YML):
    run_command = 'python3 ../src/InPheRNo_simple.py -run_directory ./run_dir -run_file ' + BENCHMARK_YML
    ...
    for f in All_files_in_results_dir:
        for BENCHMARK_name in BENCHMARK_name_list:
            if BENCHMARK_name in f:
                ...
                BENCHMARK = os.path.join(verification_dir, option, BENCHMARK_name + '.csv')
...
def main():
    BENCHMARK = {
        'full': ['InPheRNo_simplified_full.yml',
                 'Network_pvalue',
                 'Network_statistic'
        ]
    }
